...
using Strem.Plugins.Twitter.Models;
using Strem.Plugins.Twitter.Types;
using Strem.Plugins.Twitter.Variables;

namespace Strem.Plugins.Twitter.Services.OAuth;
...
public class TwitterOAuthClient : ITwitterOAuthClient
{
    ...
    public void StartAuthorisationProcess(string[] requiredScopes)
    {
        ...
        var queryData = $"client_id={AppState.GetTwitterClientId()}&redirect_uri={OAuthCallbackUrl}&response_type=code&scope={scopeQueryData}&state={randomState}&{challengeData}";
    ...
    public async Task<bool> GetToken(string code)
    {
        ...
        var clientContent = $"client_id={AppState.GetTwitterClientId()}&redirect_uri={OAuthCallbackUrl}";
    ...
    public async Task<bool> RefreshToken()
    {
        ...
        var clientContent = $"client_id={AppState.GetTwitterClientId()}";
    ...
    public async Task<bool> RevokeToken()
    {
        ...
        restRequest.AddHeader("Content-Type", "application/x-www-form-urlencoded");
        restRequest.AddStringBody($"client_id={AppState.GetTwitterClientId()}&token={accessToken}&token_type_hint=access_token", DataFormat.None);
